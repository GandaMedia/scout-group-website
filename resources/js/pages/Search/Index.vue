    ...
    <AppLayout>
        ...
        <main class="mx-auto min-h-[55vh] max-w-5xl px-4 py-12 lg:px-0">
            ...
            <form
                ...
                method="get"
                class="mt-8 flex max-w-2xl border border-slate-300 bg-white focus-within:border-scout-purple"
            >
                <Search class="mt-3.5 ml-4 size-5 shrink-0 text-slate-400" />
                <input
                    name="q"
                    type="search"
                    :value="query"
                    minlength="2"
                    maxlength="100"
                    class="min-w-0 flex-1 px-3 py-3 outline-none"
            ...
            <p v-if="query.length === 1" class="mt-6 text-slate-600">
            ...
            <div v-else-if="query.length >= 2" class="mt-10">
                ...
                >
                    ...
                    >
                        <Link
                            :href="result.href"
                            class="block px-2 py-5 hover:bg-slate-50"
                        >
                            <span
                                class="text-xs font-black text-scout-purple uppercase"
                                >{{ result.type }}</span
                            >
                            <h2 class="mt-1 text-xl font-black text-scout-navy">
                                {{ result.title }}
                            </h2>
                        </Link>
                    </li>
                ...
                <p
                    v-else
                    class="mt-6 border-l-4 border-scout-yellow bg-yellow-50 p-5 text-slate-700"
                >
